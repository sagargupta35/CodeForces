 ...
 public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        for (int i = 0; i < n; i++) {
            ...
            f1.put(arr[i], j+1);
            f2.merge(j+1, 1, Integer::sum);
            if(f1.size() == 1){
                ans = i+1;
            } else if(f2.size() == 2){
                var list = new ArrayList<>(f2.entrySet());
                // equate first to second (remove element from first)
                if(list.get(0).getValue() == 1){
                    if(list.get(0).getKey() == 1 || list.get(0).getKey() == list.get(1).getKey()+1){
                        ans = i+1;
                    }
                }
                // equate second to first
                if(list.get(1).getValue() == 1){
                    if(list.get(1).getKey() == 1 || list.get(1).getKey() == list.get(0).getKey()+1){
                        ans = i+1;
                    }
                }
            } else if(f2.size() == 1){
