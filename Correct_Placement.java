...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while(t-- > 0){
            int n = in.nextInt();
            Pair<Integer, int []> [] h = new Pair[n];
            Pair<Integer, int []> [] w = new Pair[n];
            for (int i = 0; i < n; i++) {
                h[i] = new Pair<>(i, new int [] {in.nextInt(), in.nextInt()});
                w[i] = h[i];
            }
            Arrays.sort(h, Comparator.comparingInt(p -> p.s[0]));
            Arrays.sort(w, Comparator.comparingInt(p -> p.s[1]));
            Pair<Integer, Integer> [] mh = new Pair[n];
            Pair<Integer, Integer> [] mw = new Pair[n];
            int pre = Integer.MAX_VALUE, idx = -1;
            for (int i = 0; i < n; i++) {
                if(h[i].s[1] < pre){
                    pre = h[i].s[1];
                    idx = h[i].f;
                }
                mh[i] = new Pair<>(pre, idx);
            }
            ...
            for (int i = 0; i < n; i++) {
                ...
                mw[i] = new Pair<>(pre, idx);
            }
            int [] res = new int [n];
            Arrays.fill(res, -1);
            for (int i = 0; i < n; i++) {
                int j = binarySearch(h, h[i].s[0], 0);
                if(j != -1){
                    if(mh[j].f < h[i].s[1]){
                        res[h[i].f] = mh[j].s;
                    }
            ...
            for (int i = 0; i < n; i++) {
                if(res[h[i].f] != -1) continue;
                int j = binarySearch(w, h[i].s[0], 1);
                if(j != -1){
                    if(mw[j].f < h[i].s[1]){
                        res[h[i].f] = mw[j].s;
                    }
    ...
    static int binarySearch(Pair<Integer, int []> [] arr, int target, int c){
