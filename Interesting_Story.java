...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            int [] arr = new int [5];
            int n = in.nextInt();
            Comparator<Pair<Integer, Integer>> c = (p1, p2) -> {
                int d1 = p1.f - p1.s;
                int d2 = p2.f - p2.s;
                if(d1 < d2) return -1;
                if(d1 > d2) return 1;
            ...
            for(int i = 0; i<n; i++){
                ...
                for(int j = 0; j<5; j++){
                    arr[j] += temp[j];
                    map.get((char)(j+'a')).add(new Pair<>(temp[j], s.length()-temp[j]));
                }
            }
            int max = 0;
            int sum = 0;
            ...
            for(int i = 0; i<5; i++){
                int k = n, cur = arr[i], oth = sum-cur;
                List<Pair<Integer, Integer>> list = map.get((char)(i+'a'));
                list.sort(c);
                for(var p : list){
                    if(p.f >= p.s) break;
                    if(cur > oth) break;
