 ...
 public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            Boolean [][] dp = new Boolean[m][sub.length()];
            if(f(sub, 0, l, r, dp, 0)){
    ...
    static boolean f(String sub, int i, String l, String r, Boolean [][] dp, int cur){
        if(cur == sub.length()) return true;
        if(dp[i][cur] != null) return dp[i][cur];
        int n = l.length();
        boolean flag = false;
        for(char j = l.charAt(i); j<= r.charAt(i); j++){
            int temp = cur;
            while (temp < sub.length() && sub.charAt(temp) != j) temp++;
            if(i == n-1){
                if(temp >= sub.length()){
                    flag = true;
                    break;
                ...
                if(f(sub, i+1, l, r, dp, temp < sub.length() ? temp+1 : sub.length())){
                    flag = true;
                    break;
                }
            }
        }
        return dp[i][cur] = flag;
    }
