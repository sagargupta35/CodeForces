...
public class Main {
    ...
    static int isPossible(String s, int k){
        for (int i = 0; i <= s.length() - k; i++) {
            // start of tandem
            int temp = i;
            for (int j = i; j < i + (k/2); j++) {
                if(s.charAt(j) != '?' && s.charAt(j+(k/2)) != '?' && (s.charAt(j) != s.charAt(j+(k/2)))){
                    i = j;
                    break;
                }
            }
            if(i == temp && !(s.charAt(i) != '?' && s.charAt(i+(k/2)) != '?' && (s.charAt(i) != s.charAt(i+(k/2))))){
                return i;
            }
        }
        return -1;
