...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0) {
            ...
            for(int i = 0; i<l; i++){
                ...
                vma = Math.max(vma, v);
                vmi = Math.min(vmi, v);
                // can he execute ith command
                int left = 1 + Math.abs(hmi);
                int right = m - hma;
                if(left > right) break;
                c = right;
                int up = 1 + Math.abs(vmi);
                int down = n - vma;
                if(up > down) break;
                r = down;
            }
    ...
    static boolean isPossible(int [] a, int [] b, int num){
        int ans = 0, n = a.length;
        for (int i = 0; i < n; i++) {
            if(ans <= b[i] && (num-ans-1) <= a[i]) ans++;
        }
        return ans >= num;
    }
