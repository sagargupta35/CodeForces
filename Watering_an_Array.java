...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            int [] v = ai(k);
            long max = 0;
            for (int i = 0, cur = 0; i < Math.min(d, 2 * n + 5); i++, cur = (cur+1)%k) {
                long temp = count(arr);
                max = Math.max(max, temp + (d-i-1)/2);
                plus(arr, v[cur]);
            }
    ...
    static void plus(int [] arr, int max){
        for (int i = 0; i < max; i++) {
            arr[i]++;
        }
    }
    ...
    static int f(int st, int [] dp, long [] pre, long x){
        int n = pre.length;
        if(st >= n) return 0;
        if(dp[st] != -1) return dp[st];
        long prev = 0;
        if(st > 0) prev = pre[st-1];
        int r = lowerBound(st, pre, prev+x+1);
        if(r == n) return dp[st] = 0;
        if(r == n-1) return dp[st] = 1;
        int next = f(r+1, dp, pre, x);
        return dp[st] = 1 + next;
    }

    static int lowerBound(int st, long [] pre, long target){
        int n = pre.length;
        int ans = n;
        int lo = st, hi = n-1;
        while (lo <= hi){
