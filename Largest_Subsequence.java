...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            List<Integer> list = new ArrayList<>(sbr.length());
            for(int i = 0; i<n; i++){
                if(list.isEmpty()){
                    list.add(i);
                } else{
                    while (!list.isEmpty() && sbr.charAt(list.get(list.size()-1)) < sbr.charAt(i)) list.remove(list.size()-1);
                    list.add(i);
                }
            }
            int count = 0;
            for(int i = 0; i<list.size()-1; i++){
                if(sbr.charAt(list.get(i)) == sbr.charAt(list.get(i+1))) count++;
                else break;
            }
            for(int i = 0; i<list.size()/2; i++){
                swap(sbr, list.get(i), list.get(list.size()-i-1));
            }
            if(isSorted(sbr)){
                if(list.size() == 1) System.out.println(0);
                else{
                    System.out.println(Math.max(0, list.size()-1-count));
                }
    ...
    static boolean isSorted(StringBuilder sbr){
        for(int i = 1; i<sbr.length(); i++){
            if(sbr.charAt(i-1) > sbr.charAt(i)) return false;
        }
        return true;
    }
