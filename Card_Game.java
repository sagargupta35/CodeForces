...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        for(int i = 1; i<61; i++) Arrays.fill(dp[i], -1);;
        while (t-- > 0) {
            int n = in.nextInt();
            long win = getWays(n, 1, dp);
            long lose = getWays(n, 0, dp);
            long draw = nCrModPFermat(n, n / 2, 998244353) - win - lose;
            if(draw < 0) draw += 998244353;
            System.out.println(win + " " + lose + " " + draw);
        }
        out.close();
    }

    static long getWays(int n, int win, long [][] dp){
        if(n == 0) return 0;
        if(n == 2){
            if(win == 1) return 1;
            return 0;
        }
        if(dp[n][win] != -1) return dp[n][win];
        if(win == 1){
            return dp[n][win] = ((nCrModPFermat(n-1, n/2, 998244353) + getWays(n-2, 0, dp)))%998244353;
        } else{
            return dp[n][win] = ((nCrModPFermat(n-2, n/2, 998244353) + getWays(n-2, 1, dp)))%998244353;
        }
    }
