...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            BiFunction<Character, Character, Boolean> f = (c1, c2) -> (c1 == 'a' && c2 == 'b') || (c1 == 'b' && c2 == 'c');
            int b = 0, c= 0;
            boolean flag = true;
            for(int i = 0; i<n; i++){
                if(s1.charAt(i) != s2.charAt(i)){
                    if(!f.apply(s1.charAt(i), s2.charAt(i))){
                        flag = false;
                        break;
                    }
                    if(s1.charAt(i) == 'a'){
    ...
    static int find(StringBuilder s, char c, int st){
        int ans = -1;
        while (st < s.length()){
            if(s.charAt(st) == c) return st;
            if(s.charAt(st) != (char) (c-1)) return -1;
            st++;
        }
        return ans;
    }
    ...
    static int getPersons(int [] arr, int hours){
        int tasks = 0;
        for (int j : arr) {
            if (j > hours) {
                tasks += j - hours;
                ...
                int can = (hours - j) / 2;
                tasks = Math.max(0, tasks - can);
            }
        }
        if(tasks > 0){
            hours -= (hours&1);
            if(hours == 0) return Integer.MAX_VALUE;
            tasks *= 2;
            return (int)Math.ceil(tasks / (float)hours);
        }
        return 0;
    }
