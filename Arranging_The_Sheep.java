...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            suf[n-1] = list.get(n-1);
            for(int i = n-2; i>=0; i--){
                suf[i] += suf[i+1] + list.get(i);
            }
            long min = suf[1] - ((long) (n - 1) * list.get(0)) - sumOfNumbers(0, n-1);
            for(int i = 1; i<n-1; i++){
                long cost = suf[i+1] - ((long) (n - i - 1) * list.get(i)) - sumOfNumbers(0, n-i-1) + ((long) i * list.get(i)) - sumOfNumbers(0, i) - pre[i-1];
                min = Math.min(min, cost);;
            }
            long last = (long) (n - 1) * (list.get(n-1)) - sumOfNumbers(0, n-1) - pre[n-2];
