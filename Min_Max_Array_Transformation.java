...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            int [] arr = ai(n);
            int [] b = ai(n);
            //for min find lower bound of each of arr[i] in the left half of b
            int [] min = new int [n];
            for(int i = 0; i<=n-1; i++){
                min[i] = Math.min(b[i]-arr[i], lowerBound(b, 0, i-1, arr[i]));
            }
            int [] compel = new int [n];
            for(int i = 0; i<n-1; i++){
                if(b[i] < arr[i+1]) compel[i]++;
            }
            for(int i = 1; i<n; i++) compel[i] += compel[i-1];
            int first = lBound(compel, 0, n-1, 1);
            int [] max = new int [n];
            max[0] = b[first] - arr[0];
            for(int i = 1; i<n-1; i++){
                int up = lBound(compel, i, n-1, compel[i-1]+1);
                max[i] = b[up] - arr[i];
            }
            max[n-1] = b[n-1] - arr[n-1];
            for(int i = 0; i<n; i++) System.out.print(min[i] + " ");
            System.out.println();
            for(int j : max) System.out.print(j + " ");
            System.out.println();
        }
        out.close();
    }

    static int lBound(int [] arr, int lo, int hi, int target){
        int ans = arr.length-1;
        while (lo <= hi){
            int mid = lo + (hi-lo)/2;
            if(arr[mid] >= target){
                if(arr[mid] == target) ans = Math.min(ans, mid);
                hi = mid-1;
            } else lo = mid+1;
        }
        return ans;
    ...
    static int lowerBound(int [] arr, int lo, int hi, int target){
        ...
        while (lo <= hi){
            ...
            if(arr[mid] > target){
                ans = Math.min(ans, arr[mid]-target);
                hi = mid-1;
            } else lo = mid+1;
        }
        return ans;
