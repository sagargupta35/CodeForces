...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while(t-- > 0){
            int n = in.nextInt();
            int [][] arr = new int [n][];
            Arrays.setAll(arr, i -> ai(4));
            NavigableMap<Double, int []> map = new TreeMap<>();
            for (int i = 0; i < n; i++) {
                double xd = (arr[i][2] - arr[i][0]);
                double yd = (arr[i][3] - arr[i][1]);
                if(xd == 0){
                    map.putIfAbsent(Double.MAX_VALUE, new int [] {0, 0});
                    int [] cur = map.get(Double.MAX_VALUE);
                    if(yd > 0) cur[0]++;
                    else cur[1]++;
                    continue;
                }
                double slope = yd / xd;
                if(slope == 0.0 || slope == -0.0) slope = 0;
                map.putIfAbsent(slope, new int [] {0, 0});
                int [] cur = map.get(slope);
                if(xd > 0) cur[0]++;
                else cur[1]++;
            }
            long sum = 0;
