...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0) {
            ...
            char max = 'a';
            for(int i = 0; i<n; i++){
                if(sbr.charAt(i) <= max) continue;
                if(k >= sbr.charAt(i)-max){
                    k -= sbr.charAt(i)-max;
                    max = sbr.charAt(i);
                } else{
                    setAll(sbr, (char)(sbr.charAt(i)-k), sbr.charAt(i), i);
                    break;
                }
            }
            setAll(sbr, 'a', max, 0);
    ...
    static void setAll(StringBuilder sbr, char lo, char hi, int st){
        for(int i = st; i<sbr.length(); i++){
            if(sbr.charAt(i) >= lo && sbr.charAt(i) <= hi) sbr.setCharAt(i, lo);
        }
    }
