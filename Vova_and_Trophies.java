 ...
 public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (i < n){
            if(s.charAt(i) == 'G'){
                int cur = i;
                while (i < n && s.charAt(i) == 'G') i++;
                max = Math.max(max, i - cur + 1);
                list.add(new Pair<>(cur, i-1));
            } else i++;
        }
        if(list.isEmpty()) System.out.println(0);
        else if(list.size() == 1) System.out.println(max-1);
        else if(list.size() == 2){
            if(list.get(1).f == list.get(0).s + 2){
                max = list.get(1).s - list.get(0).f;
            }
            System.out.println(max);
        } else{
            for (int j = 0; j < list.size()-1; j++) {
                if(list.get(j).s == list.get(j+1).f - 2){
                    max = Math.max(max, list.get(j+1).s - list.get(j).f + 1);
                }
            }
            System.out.println(max);
        }
