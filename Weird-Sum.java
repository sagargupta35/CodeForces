...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        for(int i = 0; i<n; i++){
            for(int j = 0; j<m; j++){
                int c = in.nextInt();
                rows.putIfAbsent(c, new ArrayList<>());
                columns.putIfAbsent(c, new ArrayList<>());
                rows.get(c).add(i);
                columns.get(c).add(j);
            }
        }
        long sum = getSum(rows);
        sum += getSum(columns);
        System.out.println(sum);
        out.close();
    }

    static long getSum(Map<Integer, List<Integer>> map){
        long sum = 0;
        for(var entry: map.entrySet()){
            List<Integer> cur = entry.getValue();
            cur.sort(Comparator.naturalOrder());
            for(int i = 0; i<cur.size(); i++){
                long t1 = i - (cur.size() - i - 1);
                sum += t1 * cur.get(i);
            }
        }
        return sum;
