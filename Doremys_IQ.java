...
public class CodeChef3 {
    ...
    static Pair<Boolean, StringBuilder> getMax(int [] arr, int cost, int el){
        int n = arr.length;
        el = n - el;
        StringBuilder sbr = new StringBuilder();
        for(int i = 0; i<el && i<n; i++){
            if(cost >= arr[i]){
                el++;
                sbr.append(1);
            } else sbr.append(0);
        }
        boolean flag = isPossible(arr, el, cost);
        if(flag){
            if(el < n){
                sbr.append("1".repeat(n-el));
            }
            return new Pair<>(true, sbr);
        }
        return new Pair<>(false, null);
    }

    static boolean isPossible(int [] arr, int st, int cost){
        int n = arr.length;
        for(int i = st; i<n; i++){
            if(cost < arr[i]) cost--;
        }
