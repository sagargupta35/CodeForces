...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            int [] arr = ai(n);
            for(int i = 0; i<m; i++){
                bmap.merge(in.nextInt(), 1, Integer::sum);
            }
            int cur = 0, sum = 0;
            Map<Integer, Integer> map = new HashMap<>();
            for(int i = 0; i<m; i++){
                Integer temp = bmap.get(arr[i]);
                if(temp != null){
                    int j = map.merge(arr[i], 1,Integer::sum);
                    if(j <= temp) cur++;
                }
            }
            if(cur >= k) sum++;
            for(int i = m; i<n; i++){
                Integer temp1 = bmap.get(arr[i-m]);
                Integer temp2 = bmap.get(arr[i]);
                if(temp1 != null){
                    int j = map.compute(arr[i-m], (key, val) -> val-1);
                    if(j < temp1) cur--;
                }
                if(temp2 != null){
                    int j = map.merge(arr[i], 1, Integer::sum);
