 ...
 public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            res.append(sbr.toString().repeat(n/fac));
            if(n%fac != 0){
                res.append(sbr.substring(0, n%fac));
            }
    ...
    static int maxRemoval(Map<Integer, List<Integer>> map, int [] arr, int st, Integer [] dp){
        if(dp[st] != null) return dp[st];
        if(st == 0){
            if(arr[st] == 1 ) return 1;
            return 0;
        }
        int ans = maxRemoval(map, arr, st-1, dp);
        List<Integer> list = map.get(arr[st]);
        if(list == null){
            return dp[st] = maxRemoval(map, arr, st-1, dp);
        } else{
            for(int j : list){
                if(j >= st) break;
                ans = Math.max(ans, maxRemoval(map, arr, j-1, dp) + st - j+1);
            }
            return dp[st] = ans;
        }
    }
