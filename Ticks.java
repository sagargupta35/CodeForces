...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0) {
            ...
            for(int i = 0; i<n; i++){
                for(int j = 0; j<m; j++){
                    if(arr[i].charAt(j) == '*'){
                        isPossible(i, j, arr, true, k, k, vis);
                        if(!vis[i][j]){
                            flag = false;
                            break;
                        }
    ...
    static boolean isPossible(int i, int j, String [] arr, boolean flag, int d, int k, boolean [][] vis){
        int n = arr.length, m = arr[0].length();
        if(i < 0 || i >= n || j < 0 || j >= m) return false;
        if(arr[i].charAt(j) != '*') return false;
        if(flag){
            boolean f2 = isPossible(i+1, j+1, arr, true, d-1, k, vis);
            if(f2){
                vis[i][j] = true;
            }
            boolean f1 = (d <= 0) && isPossible(i-1, j+1, arr, false, d+1, k, vis);
            if(f1){
                vis[i][j] = true;
            }
            return f1 | f2;
            ...
            boolean f1 = isPossible(i-1, j+1, arr, false, d+1, k, vis);
            if(f1){
                vis[i][j] = true;
                return true;
            }
