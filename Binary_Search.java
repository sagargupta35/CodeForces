...
public class Main {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        if(flag){
            long temp = nCrModPFermat(n-x, b1, mod);
            long [] arr = {nCrModPFermat(x-1, s1, mod),  nCrModPFermat(n-x, b1, mod), getFac(s1+b1), getFac(s2), getFac(b2)};
            long ans = 1;
            for(long j : arr){
                ans = (ans * j)%mod;
            }
    ...
    static int getMinK(long n, long [] facs, Map<Long, Integer> dp, boolean [] taken){
        if(n <= 0) return 0;
        if(dp.get(n) != null) return dp.get(n);
        int min = getBits(n);
        int idx = binarySearch(facs, n);
        while (idx >= 0) {
            if(!taken[idx]){
                taken[idx] = true;
                min = Math.min(min, 1+getMinK(n - facs[idx], facs, dp, taken));
                taken[idx] = false;
            }
            idx--;
        }
