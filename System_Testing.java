...
import java.util.*;
 ...
 public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
     ...
     private static void systemTesting() {
         int n = in.nextInt();
         int k = Math.min(n, in.nextInt());
         int [] arr = ai(n);
         List<Pair<Integer, Integer>> time = new ArrayList<>(List.of(new Pair<>(0, 0)));
         ...
         while (!pq.isEmpty()){
             var cur = pq.poll();
             com++;
             int per = (int) (((100*com)/(float)n) + 0.5);
             time.add(new Pair<>(cur, per));
             if(next < n){
                 pq.add(cur + arr[next]);
                 finish[next] = cur;
                 next++;
         ...
         for (int i = 0; i < n; i++) {
             for (int j = 0; j < arr[i]; j++) {
                 int per = binarySearch(time, j + finish[i]);
                 if(per == j+1){
                     ans++;
                     break;
                 }
     ...
     static int binarySearch(List<Pair<Integer, Integer>> time, int target){
        int ans = 0;
        int lo = 0, hi = time.size()-1;
        while (lo <= hi){
            int mid = lo + (hi-lo)/2;
            if(time.get(mid).f <= target){
                ans = mid;
                lo = mid+1;
            } else hi = mid-1;
        }
        return time.get(ans).s;
    }
