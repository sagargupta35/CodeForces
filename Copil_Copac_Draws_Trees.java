...
import java.io.*;
 ...
 public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            for(int i = 0; i<n-1; i++){
                ...
                arr[u].add(new Pair<>(v, i+1));
                arr[v].add(new Pair<>(u, i+1));
            }
            int ans = 0;
            for(var p : arr[0]){
                ans = Math.max(ans, dfs(arr, p.f, p.s, 0));
            }
    ...
    static int dfs(List<Pair<Integer, Integer>>[] mat, int v, int cur, int parent){
        int ans = 0;
        for(var p : mat[v]){
            if(p.f != parent) {
                ans = Math.max(ans, dfs(mat, p.f, p.s, v) + (p.s < cur ? 1 : 0));
            }
        }
        return ans;
    }
    ...
    static void print(int [][] res){
     ...
     static int [][] fillNormal(int n, int m){
        int [][] res = new int [n][m];
        int st = 1;
        for(int i = 0; i<n; i++){
            for(int j = 0; j<m; j++){
     ...
     static int [][] fillRowWise(int n, int m){
         int [][] res = new int [n][m];
         int st = 1;
         for(int j = 0; j<m; j++){
             for(int i = 0; i<n; i++){
     ...
     static int [][] fillPrime(int n, int m){
        int [][] res = new int [n][m];
        int st = 1;
        for(int i = 0; i<n; i++){
            if(st > n*m) st = m+1;
