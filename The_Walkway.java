...
public class Main {
        ...
        while (t-- > 0){
            ...
            for(int i = 0; i<m; i++){
                if(arr[i] == 1){
                    ...
                    long temp = (arr[i] - prev - 1)/d;
                    prev = arr[i];
                    b[i] = temp +1;
                }
            }
            int last = (n - arr[m-1])/d;
            long [] suf = new long[m];
            suf[m-1] = b[m-1] + last;
            for(int i = m-2; i>=0; i--){
                suf[i] = b[i] + suf[i+1];
            ...
            long min = suf[1] - (arr[1]-arr[0]-1)/d + (arr[1]-2)/d + 1;
            int m2 = 1;
            for(int i = 1; i<m-1; i++){
                long suff = suf[i+1] - (arr[i+1] - arr[i] - 1)/d;
                long temp = suff + b[i-1] + (arr[i+1] - arr[i-1] - 1)/d;
                if(temp < min){
                    min = temp;
                    m2 = 1;
                } else if(temp == min) m2++;
            }
            long temp = b[m-2] + (n - arr[m-2])/d;
            if(temp < min){
                min = temp;
                m2 = 1;
            } else if(temp == min) m2++;
    ...
    static List<List<String>> accountsMerge(List<List<String>> accounts) {
        ...
        for(int i = 0; i<accounts.size(); i++){
            for(int j = 1; j<accounts.get(i).size(); j++){
                ...
                Integer prev = map.get(email);
                if(prev != null){
                    ds.unionBySize(i, prev);
        ...
        for(int i = 0; i<accounts.size(); i++){
            ...
            for(int j = 1; j<accounts.get(i).size(); j++){
                arr[parent].add(accounts.get(i).get(j));
            }
        }
    ...
    static class DisjointSet {
        ...
        public int findParent(int node){
            ...
            int ulp =  findParent(parents.get(node));
            parents.set(node, ulp);
            return ulp;
        }

        public void unionByRank(int u, int v){
            if(parents.get(u) == parents.get(v)) return;
            ...
            if(rank.get(ulp) < rank.get(vlp)){
                ...
                parents.set(vlp, ulp);
                int uRank = rank.get(ulp);
                rank.set(ulp, uRank+1);
            }
        }
        ...
        public void unionBySize(int u, int v){
            ...
            if(size.get(ulp) < size.get(vlp)){
                parents.set(ulp, vlp);
                size.set(vlp, size.get(vlp) + size.get(ulp));
            } else{
                parents.set(vlp, ulp);
    ...
    static String check(int [] arr, String s, int k){
        int i =0, cur = 0;
        StringBuilder res = new StringBuilder();
        while (cur < arr.length){
            int max = (1<<k)-1;
            while (arr[cur] != max && i < s.length()){
                int j = (s.charAt(i)-'a');
                if(j < k){
                    arr[cur] |= (1 << j);
                }
            ...
            if(i == s.length()){
                if(cur == arr.length - 1 && arr[cur] == max) return null;
                int j = 0;
                char c = 'a';
                while (j < k){
                    if((arr[cur] & (1 << j)) == 0){
                ...
                res.append(c);
                res.append("a".repeat(arr.length-res.length()));
                return res.toString();
            }
