...
public class Main {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            for(int i = 0; i<26; i++){
                char c = (char) ('a'+i);
                if(f[i] > 0){
                    list.add(new Pair<>(c, f[i]));
                }
            }
            list.sort(Comparator.comparingInt(p -> -p.s));
            int cost = Integer.MAX_VALUE;
            int [] res = new int [26];
            List<Integer> facs = getFactors(n);
            for(int factor : facs){
                int ch = n/factor;
                if(ch <= 26){
                    // compute the cost
                    int curCost = compute(list, ch, factor);
                    // if cost is less than the current cost, build the string
                    if(curCost < cost){
                        Arrays.fill(res, 0);
                        if(ch <= list.size()){
                            for(int i = 0; i<ch; i++){
    ...
    static int compute(List<Pair<Character, Integer>> list, int ch, int f){
        int ans = 0;
        if(ch >= list.size()){
            for(var p : list){
                if(p.s <= f) break;
