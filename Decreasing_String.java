...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            while (pos > (s.length() - cnt)){
                pos -= (s.length() - cnt);
                cnt++;
                st = update(stack, st, s);
            }
            while (st < s.length()) stack.push(s.charAt(st++));
            StringBuilder sbr = new StringBuilder();
            while (!stack.isEmpty()){
                sbr.append(stack.pop());
            }
            pos--;
            System.out.print(sbr.charAt(sbr.length() - (int) pos - 1));
        }
        out.close();
    }

    static int update(Stack<Character> stack, int st, String s){
        if(st >= s.length()){
            stack.pop();
            return st;
        }
