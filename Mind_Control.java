...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            int k = in.nextInt();
            int [] arr = ai(n);
            int rem = Math.min(m-1, k);
            int ans = Integer.MIN_VALUE;
            for (int i = 0; i <= rem; i++) {
                // remove i elements from first and rem - i elements from last
                // st = i, end = n - (rem-i) - 1
                int max = getMax(arr, i, n - (rem-i) - 1, Math.max(0, m - rem - 1));
    ...
    static int getMax(int [] arr, int st, int end, int rem){
        int min = Integer.MAX_VALUE;
        for (int i = 0; i <= rem; i++) {
            // remove i elements from start and rem-i elements from last
            int max = Math.max(arr[st+i], arr[end -  (rem-i)]);
