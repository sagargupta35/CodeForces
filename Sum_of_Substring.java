...
public class Main {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            long sum = 0;
            BiFunction<Character, Character, Integer> f = (c1, c2) -> (c1-'0')*10 + c2-'0';
            for(int i = 0; i<n-1; i++) sum += f.apply(s.charAt(i), s.charAt(i+1));
            if(sum <= 1){
                System.out.println(sum);
            } else if(sum == 10){
                int idx = s.lastIndexOf('1')+1;
                idx = n - idx;
                if(k >= idx) System.out.println(1);
                else System.out.println(10);
            } else{
                int idx;
                if (s.charAt(n - 1) != '1') {
                    idx = s.lastIndexOf('1') + 1;
                    idx = n - idx;
                    if (idx <= k) {
                        sum -= 10;
                        k -= idx;
