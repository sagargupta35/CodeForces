...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        if(n%3 > 0) len[0]++;
        if(n%3 > 1) len[1]++;
        for(int i = 0; i<3; i++) dp[i] = new int[3][len[i]];
        fill(s, dp);
        while (m-- > 0){
            int l = in.nextInt()-1;
            int r = in.nextInt()-1;
            System.out.println(find(dp, l, r, s));
        }
        out.close();
    }

    static int find(int [][][] dp, int l, int r, String s){
        if(l == r) return 0;
        if(l == r-1){
        ...
        for(int i = 0; i<3; i++){
            ...
            for(int j = 0; j<3; j++){
                // j -> char
                int k = (l+i);
                cur[i][j] = dp[k%3][j][(k/3) + (r-k)/3] - ((k < 3) ? 0 : dp[k%3][j][(k/3)-1]);
            }
        }
    ...
    static void fill(String s, int [][][] dp){
        for(int i = 0; i<3; i++) fill(s, dp, i);
    }

    static void fill(String s, int [][][] dp, int st){
        int cnt = 0, i = st;
        int [] arr = new int [3];
        while (i < s.length()){
            cnt++;
