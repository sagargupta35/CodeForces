...
public class Main {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            sort(b, a, 0, n-1);
            System.out.println(sortAndCountInversions(a, 0, n-1));
        }
        out.close();
    }

    static long mergeAndCountInversions(int [] arr, int l, int mid, int r){
    ...
    static long sortAndCountInversions(int [] arr, int lo, int hi){
        long ans = 0;
        if(lo < hi){
            int mid = lo + (hi-lo)/2;
            ans += sortAndCountInversions(arr, lo, mid);
            ans += sortAndCountInversions(arr, mid+1, hi);
            ans += mergeAndCountInversions(arr, lo, mid, hi);
        }
        return ans;
    }
