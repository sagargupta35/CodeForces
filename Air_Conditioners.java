...
public class Main {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            Pair<Integer, Integer> [] air = new Pair[n];
            int [] pre = new int [n];
            int [] suf = new int [n];
            Arrays.fill(pre, Integer.MAX_VALUE-1);
            Arrays.fill(suf, Integer.MAX_VALUE-1);
            for (int i = 0; i < n; i++) {
                if(map.containsKey(i+1)){
                    pre[i] = map.get(i+1);
                }
                if(i > 0){
                    pre[i]= Math.min(pre[i], pre[i-1]+1);
                }
            }
            for (int i = n - 1; i >=0; i--) {
                if(map.containsKey(i+1)){
                ...
                if(i < n-1){
                    suf[i] = Math.min(suf[i], suf[i+1]+1);
                }
            }
            for (int i = 0; i < n; i++) {
                System.out.print(Math.min(pre[i], suf[i]) + " ");
            }
            System.out.println();
        }
    }
