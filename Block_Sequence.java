...
public class Main {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            System.out.println(getMinOperations(arr, 0, dp));
        }
        out.close();
    }

    static int getMinOperations(int [] arr, int st, Integer [] dp){
        int n = arr.length;
        if(st == n) return 0;
        if(dp[st] != null) return dp[st];
        int removed = getMinOperations(arr, st+1, dp)+1;
        int notRemoved = Integer.MAX_VALUE;
        if(st + arr[st] + 1 <= n) notRemoved = getMinOperations(arr, st + arr[st] + 1, dp);
        return dp[st] = Math.min(removed, notRemoved);
    }
