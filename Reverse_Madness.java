...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            reduce(l, 1);
            reduce(r, 1);
            StringBuilder [] sbr = new StringBuilder[k];
            for(int i = 0; i<k; i++){
                sbr[i] = new StringBuilder();
                for(int j = l[i]; j<=r[i]; j++){
                    sbr[i].append(s.charAt(j));
                }
            }
            int q = in.nextInt();
            int [][] updates = new int [k][];
            for(int i = 0; i<k; i++){
                updates[i] = new int [r[i] - l[i] + 1];
            }
            int [] que = ai(q);
            reduce(que, 1);
            for(int i = 0; i<q; i++){
                int idx = binarySearch2(l, que[i]);
                int [] cur = updates[idx];
                int min = Math.min(que[i], r[idx] + l[idx] - que[i]);
                int max = Math.max(que[i], r[idx] + l[idx] - que[i]);
                min -= l[idx]; max -= l[idx];
                cur[max]++;
                if(min > 0) cur[min-1]--;
            }
            ...
            for(int i = 0; i < k; i++){
                ...
                for(int j = 0; j<len/2; j++){
                    if(isOdd(updates[i][j])){
                        swap(sbr[i], j, len-j-1);
                    }
                }
            }
    ...
    static void reduce(int [] arr, int j){
        for(int i = 0; i<arr.length; i++){
            arr[i] -= j;
        }
    }

    static int binarySearch2(int [] l, int target){
    ...
    public boolean compute(Node root) {
        ...
        return isPalindrome(sbr);
    }

    static boolean isPalindrome(StringBuilder sbr){
        int i = 0, j = sbr.length()-1;
        while (i < j){
            if(sbr.charAt(i) != sbr.charAt(j)) return false;
            i++;
            j--;
        }
        return true;
