...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            int n = in.nextInt();
            int [] arr = ai(n);
            NavigableMap<Integer, Pair<Integer, Integer>> map = new TreeMap<>();
            for (int i = 0; i < n; i++) {
                map.putIfAbsent(arr[i], new Pair<>(i, i));
                var p = map.get(arr[i]);
                p.s = Math.max(p.s, i);
            }
            int min = n;
    ...
    static boolean check(NavigableMap<Integer, Pair<Integer, Integer>> map, int st){
        int left = st, right = st;
        for(var entry : map.entrySet()){
            var p = entry.getValue();
            int l = p.f, r = p.s;
            int req = Math.max(0, left - l) + Math.max(0, r - right);
            if(right - left + 1 + req > entry.getKey()){
                return false;
            }
