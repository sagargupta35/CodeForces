...
class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            sum -= numDistinctEleInSubArrays(arr);
            sum += sumOfMaxInSubArrays(arr);
            System.out.println(sum);
        }
        out.close();
    }

    static long numDistinctEleInSubArrays(int [] arr){
        ...
        for(int i = 0; i<n; i++){
            int j = map2.getOrDefault(arr[i], -1);
            long d = i - j -1;
        ...
        for(var entry : map2.entrySet()){
            int k = entry.getKey();
            long d = n - entry.getValue()-1;
            d = (d * (d+1))/2;
            long finalD = d;
            map.compute(k, (key, val2) -> val2- finalD);
        }
        long sum = 0;
        for(long j : map.values()) sum += j;
        return sum;
    }

    static long sumOfMaxInSubArrays(int [] arr){
        int n = arr.length;
        int [] cli = new int [n];
        int [] cri = new int [n];
        ...
        long sum = 0;
        for(int i = 0; i<n; i++){
            sum += (long) (cli[i] + 1) *(cri[i]+1) * arr[i];
        }
        return sum;
    }
