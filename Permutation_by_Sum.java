...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            int r = in.nextInt()-1;
            int s = in.nextInt();
            List<Integer> res = get(n, s, r-l+1);
            if(res == null){
    ...
    static List<Integer> get(int n, int sum, int el){
        if(el == 1){
            if(sum <= n) return new ArrayList<>(List.of(sum));
            return null;
        }
        int max = 1;
        while (((max)*(max+1)/2) <= sum) max++;
        max--;
        if(max < el) return null;
        List<Integer> res = new ArrayList<>();
        for(int i = 1; i<el; i++) res.add(i);
        sum -= (el*(el-1))/2; // sum is always greater than el-1
        if(sum > n){
            int diff = (sum-n)/(el-1);
            res.replaceAll(integer -> integer + diff);
            sum -= diff*(el-1);
            int last = res.size()-1;
            while (sum > n){
                sum--;
                res.set(last, res.get(last)+1);
                last--;
            }
        }
        res.add(sum);
        for(int i = 0; i<res.size()-1; i++){
            if(res.get(i) >= res.get(i+1)) return null;
        }
        return res;
    }
