...
public class CodeChef3 {
            ...
            for(char c = 'a'; c <= 'z'; c++){
                ...
                while (i < n){
                    if(s1.charAt(i) != s2.charAt(i)){
                        ...
                        if(s2.charAt(i) == c){
                            q.add(new Pair<>(i, false));
                        }
                    }
                    i++;
                }
                ops += equal(q, res, s1, s2);
            }
    ...
    static int equal(Queue<Pair<Integer, Boolean>> q, StringBuilder res, StringBuilder s1, StringBuilder s2){
        int ans = 0;
        while (!q.isEmpty()){
            var p1 = q.poll();
            var p2 = q.poll();
            // p.s -> true -> belongs to s1
            if(p1.s && p2.s){
                swap(s1, s2, res, p2.f, p1.f);
                ans += 1;
            } else if(p1.s){
                swap(s1, s2, res, p2.f, p2.f);
                swap(s1,s2, res, p2.f, p1.f);
                ans += 2;
            } else if(p2.s){
                swap(s1, s2, res, p2.f, p2.f);
                swap(s1, s2, res, p1.f, p2.f);
                ans += 2;
            } else{
                swap(s1, s2, res, p1.f, p2.f);
                ans++;
    ...
    static void swap(StringBuilder s1, StringBuilder s2, StringBuilder res, int i, int j){
        res.append("\n").append(i+1).append(" ").append(j+1);
        char c = s1.charAt(i);
        s1.setCharAt(i, s2.charAt(j));
        s2.setCharAt(j, c);
    }
