...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0) {
            ...
            while (!res.isEmpty()){
                int idx = upperBound(res, (k - res.get(0))%k);
                sum += (res.get(0)+res.get(idx))/k;
                res.remove(0);
                res.remove(idx-1);
            }
    ...
    static int upperBound(List<Integer> res, int target){
        int lo = 1, hi = res.size()-1;
        int ans = -1;
        while (lo <= hi){
            int mid = lo + (hi - lo)/2;
            int j = res.get(mid);
            if(j == target) return mid;
            if(j > target){
                ans = Math.max(ans, mid);
                lo = mid+1;
            } else hi = mid-1;
        }
        return ans == -1 ? res.size()-1 : ans;
    }
