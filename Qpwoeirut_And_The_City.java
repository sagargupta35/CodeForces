...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            if((n&1) == 1) System.out.println(sum);
            else{
                long [] shift = new long [(n-1)/2];
                for(int i = 2; i<n-1; i+=2){
                    long max = Math.max(arr[i-1], arr[i+1]);
                    long inc = 0;
                    if(arr[i] <= max) inc = (max-arr[i])+1;
                    shift[(i-1)/2] = inc;
                }
                for(int i = shift.length-2; i>=0; i--){
                    shift[i] += shift[i+1];
                }
                System.out.println(getMinFloors(arr, 1, shift));
            }
        }
        out.close();
    }

    static long getMinFloors(int [] arr, int st, long [] shift){
        int n = arr.length-1;
        if(st >= n-1) return 0;
        long max = Math.max(arr[st-1], arr[st+1]);
        long inc = 0;
        if(arr[st] <= max) inc = (max-arr[st])+1;
        long next = getMinFloors(arr, st+2, shift);
        return Math.min(shift[st/2], inc + next);
    }
