...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            Pair<Long, Long> [] len = new Pair[c];
            Pair<Long, Long> [] query = new Pair[c];
            long cur = n;
            for(int i = 0; i<c; i++){
                long f = in.nextLong();
                long sec = in.nextLong();
                query[i] = new Pair<>(f, sec);
                len[i] = new Pair<>(cur+1, cur+sec-f+1);
                cur = len[i].s;
            }
            for(int i = 0; i<q; i++){
                long idx = in.nextLong();
                for(int j = c-1; j>=0; j--){
                    if(idx >= len[j].f && idx <= len[j].s){
                        long gap = idx - len[j].f;
                        idx = query[j].f + gap;
                    }
                }
                System.out.println(s.charAt((int)(idx-1)));
            }
        }
        out.close();
    }
    ...
    public static int stockBuyAndSell(int n, int[] prices) {
        ...
        for(int i = n-2; i>=0; i--){
            ...
            buy = Math.max(-prices[i] + sell, buy);
        }
        return buy;
    }
