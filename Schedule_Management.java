...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            for(int i = 0; i<m; i++) map.merge(in.nextInt(), 1, Integer::sum);
            int [] arr = new int [map.size()];
            int st = 0;
            for(var entry : map.entrySet()) arr[st++] = entry.getValue();
            Arrays.sort(arr);
            reverseArray(arr);
            int lo = 1, hi = arr[0];
            int ans = hi;
            while (lo <= hi){
                int mid = lo + (hi-lo)/2;
                int persons = getPersons(arr, mid);
                if(persons <= n-arr.length){
