...
import java.io.*;
 ...
 public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0) {
            ...
            while (q-- > 0){
                int l = in.nextInt()-1;
                long u = in.nextInt();
                long temp = l > 0 ? arr[l-1] : 0;
                long limit = temp + u;
                if(limit <= arr[l]){
                    System.out.print((l+1) + " ");
                    continue;
                }
                var floorEntry = map.floorEntry(limit);
                int idx = floorEntry.getValue();
                long val = floorEntry.getKey();
                if (idx == n - 1 || val == limit) {
                    System.out.print((idx + 1) + " ");
                    continue;
                }
                long sec = val - (l > 0 ? arr[l - 1] : 0);
                long next = arr[idx + 1] - arr[idx];
                long sum = sumOfNumbers(sec, sec + next - 1);
                u *= next;
                if (u > sum) {
    ...
    public static int getMinimumCost(int mask, int st, int [] arr, int max, int num, int k, int m, int prev){
        int n = arr.length;
        if(st > n-k) return 0;
        if(mask == 0){
            max = arr[st];
        ...
        if(num >= m){
            ...
            for(int i = st; i < st+k; i++){
                int pow = 1 << i;
                if((pow&mask) > 0){ // if arr[i] is max
                    if((pow&prev) == 0){ // if arr[i] has not previously touched
                        if(i == st){
                            min = getMinimumCost(0, st+1, arr, -1, -1, k, m, pow);
                        } else{
                            arr[i]++;
                            min = Math.min(min, getMinimumCost(pow, st+1, arr, arr[i], 1, k, m, pow));
                            arr[i]--;
                        }
                    }
                }
            ...
            return getMinimumCost(mask, st+1, arr, max, num, k, m, prev);
        }
    }
