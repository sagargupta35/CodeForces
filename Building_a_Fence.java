...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            long [] suf = check(arr, n-2, arr[n-1], arr[n-1], k);
            if(suf == null){
                System.out.println("NO");
                continue;
            }
            if(suf[0]+1-k > arr[0]) System.out.println("NO");
            else if(suf[1]+k-1 < arr[0]) System.out.println("NO");
            else System.out.println("YES");
        }
        out.close();
    }

    static long [] check(long [] arr, int st, long min, long max, int k){
        // touch or not
        // base under height
        if(min+1-k > arr[st]+k-1) return null;
         // base over height
        if(max+k-1 < arr[st]) return null;
        min = Math.max(min+1-k, arr[st]);
        max = Math.min(arr[st]+k-1, max+k-1);
        if(st == 1) return new long [] {min, max};
        return check(arr, st-1, min, max, k);
    }
