...
public class Main {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            PriorityQueue<Integer> pq = new PriorityQueue<>(Comparator.comparingInt(a -> arr[a]));
            for(int i = 0; i<n; i++){
                pq.add(i);
                if(pq.size() > x){
                    pq.poll();
            ...
            for(int i =0; i<n; i++){
                if(b[i] == 0){
                    var entry = map.ceilingEntry(arr[i]);
                    if(entry == null){
                        flag = false;
                        break;
                    }
    ...
    static boolean traverse(int level, Node root, List<Integer> res, boolean flag){
        if(level == 1){
            res.add(root.data);
            return true;
        }
        ...
        boolean left, right;
        if(flag) {
            left = traverse(level - 1, root.left, res, true);
            right = traverse(level - 1, root.right, res, true);
        } else{
            right = traverse(level - 1, root.right, res, false);
            left = traverse(level - 1, root.left, res, false);
        }
    ...
    public static List<Integer> findSpiralOrderTraversal(Node root) {
        // Write your code here...
        List<Integer> res = new LinkedList<>();
        int level = 1;
        while (traverse(level++, root, res, true) && traverse(level++, root, res, false)) {
