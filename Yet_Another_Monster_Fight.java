...
public class Main {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        long [] pre = new long [n];
        for (int i = 1; i < n; i++) {
            pre[i] = Math.max(pre[i-1]+1, arr[i-1]+1);
        }
        long [] suf = new long [n];
        for (int i = n - 2; i >= 0; i--) {
            suf[i] = Math.max(suf[i+1]+1, arr[i+1]+1);
        }
        long min = Math.min(Math.max(arr[0], suf[0]), Math.max(arr[n-1], pre[n-1]));
        for (int i = 1; i < n-1; i++) {
            long max = Math.max(arr[i], Math.max(pre[i]+n-i-1, suf[i]+i));
