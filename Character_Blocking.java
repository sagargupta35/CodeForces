...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            for(int i = 0; i<q; i++){
                ...
                if(s.charAt(0) == '1' ){
                    int pos = Integer.parseInt(s.substring(2))-1;
                    if(s1.charAt(pos) != s2.charAt(pos)) c--;
                    pq.add(new Pair<>(i, pos));
                } else if(s.charAt(0) == '2'){
                    String [] arr = s.split(" ");
                    int p1 = Integer.parseInt(arr[2])-1;
                    int p2 = Integer.parseInt(arr[4])-1;
                    c -= getNotEqual(s1, s2, p1, p2);
                    if(arr[1].equals("1")){
                        char c1 = s1.charAt(p1);
                        if(arr[3].equals("1")){
                            char c2 = s1.charAt(p2);
                            s1.setCharAt(p1, c2);
                            s1.setCharAt(p2, c1);
                        } else{
                    ...
                    while (!pq.isEmpty() && pq.peek().f <= i-ti){
                        var p = pq.poll();
                        if(s1.charAt(p.s) != s2.charAt(p.s)) c++;
                    }
    ...
    static int getNotEqual(StringBuilder s1, StringBuilder s2, int idx1, int idx2){
        int cnt = 0;
        if(s1.charAt(idx1) != s2.charAt(idx1)) cnt++;
        if(s1.charAt(idx2) != s2.charAt(idx2)) cnt++;
        return cnt;
    }
