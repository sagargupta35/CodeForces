...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0) {
            ...
            Map<Integer, Map<Integer, Map<Integer, Long>>> map = new HashMap<>();
            System.out.println(getMinCost(s, 0, n-1, 'a', map));
        }
        out.close();
    }

    static long getMinCost(String s, int l, int r, char c, Map<Integer, Map<Integer, Map<Integer, Long>>> map){
        if(l == r){
            if(s.charAt(l) == c) return 0;
            return 1;
        }
        // initialize
        map.putIfAbsent(l, new HashMap<>());
        map.get(l).putIfAbsent(r, new HashMap<>());
        // check for presence
        if(map.get(l).get(r).containsKey(c-'a')) return map.get(l).get(r).get(c-'a');
        // first half
        int f = 0;
        int len = (r - l + 1)/2;
        ...
        long first = f + getMinCost(s, l+len, r, (char) (c+1), map);
        // second
        int sec = 0;
        for(int i = r; i>r-len; i--){
            if(s.charAt(i) != c) sec++;
        }
        long second = sec + getMinCost(s, l, r-len, (char) (c+1), map);
        map.get(l).get(r).put(c-'a', Math.min(first, second));
        return Math.min(first, second);
    }

    static int getCnt(String s, int st, String t, int idx){
        if(st >= s.length()) return 0;
        if(s.charAt(st) == t.charAt(idx)) return 1 + getCnt(s, st+1, t, 1-idx);
        return getCnt(s, st+1, t, idx);
    }
    ...
    private static Node sorted(int [] nums, int l, int r){
        if(l > r) return null;
    ...
    static int lowerBound(int [] arr, int st, int end, int target){
        int ans = st;
        int lo = st, hi = end;
        while (lo <= hi){
            int mid = lo + (hi - lo)/2;
