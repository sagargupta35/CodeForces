...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0) {
            ...
            System.out.println(numPairs(s, s.length()-1, false, false, dp)-2);
        }
        out.close();
    }

    static int numPairs(String s, int st, boolean f1, boolean f2, int [][] dp){
        if(st < 0){
            if(!f1 && !f2) return 1;
            return 0;
        }
        int cur = s.charAt(st)-'0';
        if(f2) cur--;
        if(cur < 0) cur += 10;
        int res = 0;
        if(f2 && cur == 9){
            return 10 * numPairs(s, st-1, true, f1, dp);
        }
        res += dp[cur][1] * numPairs(s, st-1, true, f1, dp);
        res += dp[cur][0] * numPairs(s, st-1, false, f1, dp);
        return res;
    }

    static void fill(int [][] dp){
        // dp.length = 10, dp[0].length = 2
        for(int i = 0; i<10; i++){
            for(int j = 0; j < 10; j++){
                int k = (i+j);
                int rem = k%10;
