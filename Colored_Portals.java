...
public class Main {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            Function<String, Integer> idx = city -> {
                int cur = 0;
                for (int j = 0; j < 2; j++) {
                    cur |= 1 << fun.apply(city.charAt(j));
                }
                return cur;
            };
            for(int i = 0; i<n; i++){
                String city = arr[i];
                int cur = idx.apply(city);
            ...
            while (q-- > 0){
                ...
                int c1 = idx.apply(arr[l]);
                int c2 = idx.apply(arr[r]);
                int dist = Integer.MAX_VALUE;
                for (int i = 3; i <= 12; i++) {
                    if(can(i, c1) && can(i, c2)){
                        List<Integer> list = map.get(i);
                        if(list != null){
                            int lb = lowerBound(list, r);
                            int ub = upperBound(list, l);
                            if(lb != -1){
                                dist = Math.min(dist, Math.abs(lb - l) + Math.abs(lb - r));
                            }
                            if(ub != -1){
                                dist = Math.min(dist, Math.abs(ub - l) + Math.abs(ub - r));
                            }
                        }
                    }
                }
    ...
    static boolean can(int i, int j){
        int cur = 1;
        while (cur <= i && cur <= j){
            if((cur&i) > 0 && (cur&j) > 0) return true;
            cur *= 2;
        }
        return false;
    }

    static List<Integer> getPrimeFactors(int n, int [] sieve){
