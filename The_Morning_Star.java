    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            for(int i = 0; i<n; i++){
                int x1 = in.nextInt();
                int y1 = in.nextInt();
                x.merge(x1, 1, Integer::sum);
                y.merge(y1, 1, Integer::sum);
                plus.merge(x1+y1, 1, Integer::sum);
                minus.merge(y1-x1, 1, Integer::sum);
            }
            long sum = 0;
            ...
            for(var entry: plus.entrySet()){
                long val = entry.getValue();
                sum += val * (val-1);
            }
            for(var entry: minus.entrySet()){
                long val = entry.getValue();
                sum += val * (val-1);
            }
