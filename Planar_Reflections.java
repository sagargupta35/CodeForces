 ...
 public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            decay(k, 1, 0, n, dp);
            System.out.println(1 + dp[k][1][0]);
        }
    }

    static long decay(int age, int dir, int plane, int n, long [][][] dp){
        if(plane < 0 || plane >= n) return 0;
        if(dp[age][dir][plane] != -1) return dp[age][dir][plane];
        long sum = 0;
        if(age > 1){
            if(dir == 0){
                sum += 1 + decay(age-1, 1, plane+1, n, dp);
                sum %= mod;
                sum += decay(age, 0, plane-1, n, dp);
            } else{
                sum += 1 + decay(age-1, 0, plane-1, n, dp);
                sum %= mod;
                sum += decay(age, 1, plane+1, n, dp);
            }
        }
        return dp[age][dir][plane] = sum%mod;
    }
