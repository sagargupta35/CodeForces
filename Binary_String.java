...
public class CodeChef3 {
    ...
    static boolean isPossible(String s, int cost){
        int o = 0;
        int z = 0, left = 0, n = s.length();
        for(int i = 0; i<n; i++) if(s.charAt(i) == '1') o++;
        for(int i = 0; i<n; i++){
            if(Math.max(o, z) <= cost) return true;
            if(s.charAt(i) == '0') z++;
            else o--;
            if(z > cost){
                while (left <= i){
                    if(s.charAt(left) == '0') z--;
                    else o++;
                    left++;
                    if(z <= cost) break;
                }
        ...
        return Math.max(o, z) <= cost;
    }

    static long sufSum(List<Integer> list){
        long sum = 0;
