...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            for(int i = 0; i<n; i++){
                arr[i] = new StringBuilder(in.nextLine());
            }
            StringBuilder sbr = new StringBuilder();
            int cnt = 0;
            for(int i = 0; i<n-1; i++){
                for(int j = 0; j<m-1; j++){
                    List<Pair<Integer, Integer>> l0 = new ArrayList<>();
                    List<Pair<Integer, Integer>> l1 = new ArrayList<>();
                    for(int k1 = 0; k1<2; k1++){
                        for(int k2 = 0; k2<2; k2++){
                            if(arr[i+k1].charAt(j+k2) == '0'){
                                l0.add(new Pair<>(i+k1+1, j+k2+1));
                            } else{
                                l1.add(new Pair<>(i+k1+1, j+k2+1));
                            }
                            arr[i+k1].setCharAt(j+k2, '0');
                        }
                    }
                    switch (l1.size()){
                        case 1 -> {
    ...
    static void solve4(StringBuilder sbr, List<Pair<Integer, Integer>> min){
        List<Pair<Integer, Integer>> max= new ArrayList<>();
        for(int i = 0; i<3; i++){
            var p = min.remove(0);
            sbr.append(p).append(" ");
            max.add(p);
    ...
    static void solve1(StringBuilder sbr, List<Pair<Integer, Integer>> min, List<Pair<Integer, Integer>> max){
        var p1 = max.remove(0);
        var p2 = max.remove(0);
        var p3 = min.remove(0);
        min.addAll(List.of(p1, p2));
        max.add(p3);
        sbr.append(p1).append(" ").append(p2).append(" ").append(p3).append("\n");
        solve2(sbr, min, max);
    }

    static void solve2(StringBuilder sbr, List<Pair<Integer, Integer>> min, List<Pair<Integer, Integer>> max){
        var p1 = min.remove(0);
        var p2 = max.remove(0);
        var p3 = max.remove(0);
        sbr.append(p1).append(" ").append(p2).append(" ").append(p3).append("\n");
        min.addAll(List.of(p2, p3));
        solve3(sbr, min);
    }

    static void solve3(StringBuilder sbr, List<Pair<Integer, Integer>> min){
        for (Pair<Integer, Integer> pair : min) {
            sbr.append(pair).append(" ");
        }
        sbr.append("\n");
    }
