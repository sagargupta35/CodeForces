...
public class Main {
    ...
    static boolean isKRepeating(String s, int len){
        return isStartKRepeating(s, len) || isEndKRepeating(s, len);
    }

    static boolean isStartKRepeating(String s, int len){
        String sub = s.substring(0, len);
        int ans = 0;
        for(int i = len; i < s.length(); i += len){
            ans += numMisMatch(s, i, sub);
        }
        return ans <= 1;
    }

    static boolean isEndKRepeating(String s, int len){
        int n = s.length();
        String sub = s.substring(n - len);
        int ans = 0;
    ...
    static int numMisMatch(String s, int st, String sub){
        int ans = 0;
        for(int i = st; i < st + sub.length(); i++){
            if(s.charAt(i) != sub.charAt(i-st)) ans++;
        }
        return ans;
    }
