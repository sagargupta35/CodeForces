...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            int [] b = ai(n);
            int [] c = ai(m);
            Map<Integer, Queue<Integer>> m1 = new HashMap<>();
            Map<Integer, Integer> m2 = new HashMap<>();
            for (int i = 0; i < n; i++) {
                m2.putIfAbsent(b[i], i);
                m1.putIfAbsent(b[i], new LinkedList<>());
                if(a[i] != b[i]){
                    m1.get(b[i]).add(i);
                }
            }
            ...
            for (int i = 0; i < m; i++) {
                if(m2.containsKey(c[i])){
                    var cur = m1.get(c[i]);
                    if(!cur.isEmpty()){
                        int k = cur.poll(); // plank that will be colored by c[i]
                        res[i] = k;
                        a[k] = c[i];
                    } else{
                        res[i] = m2.get(c[i]);
                    ...
                    var cur = m1.get(c[dummy]);
                    if(cur.isEmpty()){
                        cur.add(m2.get(c[dummy]));
                    }
                    res[i] = cur.peek();
                }
            }
