...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while(t-- > 0){
            ...
            int res = vh;
            for (int i = 1; i < n - 1; i++) {
                int pre = isInt(arr, i) ? 1 : 0;
                if(i > 1 && isInt(arr, i-1)) pre++;
                if(i < n-2 && isInt(arr, i+1)) pre++;
                int cur = Integer.MAX_VALUE;
                for(int j : List.of(Integer.MIN_VALUE, arr[i-1], arr[i+1], Integer.MAX_VALUE)){
                    cur = Math.min(cur, check(arr, i, j));
                }
                res = Math.min(res, vh - pre + cur);
            }
            System.out.println(res);
        }
    }

    static int check(int [] arr, int i, int val){
        int temp = arr[i];
        arr[i] = val;
        int ans = 0;
        if(isInt(arr, i)) ans++;
        if(i > 1 && isInt(arr, i-1)) ans++;
        if(i < arr.length-2 && isInt(arr, i+1)) ans++;
        arr[i] = temp;
        return ans;
    }

    static boolean isInt(int [] arr, int idx){
        return (arr[idx] > arr[idx+1] && arr[idx] > arr[idx-1]) || (arr[idx] < arr[idx-1] && arr[idx] < arr[idx+1]);
    }
