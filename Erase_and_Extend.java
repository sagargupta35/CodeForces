...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        print(s.substring(0, i), k);
    }

    static void print(String s, int k){
        StringBuilder sbr = new StringBuilder(s.repeat((k/s.length())+1));
        while (sbr.length() > k) sbr.deleteCharAt(sbr.length()-1);
        System.out.println(sbr);
    }

    static boolean cut(String s, int st){
        int left = 0, right = st;
        boolean flag = true;
        while (left < st && right < s.length()){
