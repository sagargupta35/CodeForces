...
public class Main {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            Pair<Integer, Integer> [] lr = new Pair[n];
            for (int i = 0; i < n; i++) {
                lr[i] = new Pair<>(in.nextInt(), in.nextInt());
            }
            int [] ops = new int [1];
            dfs(adj, 0, ops, lr);
    ...
    static long dfs(List<Integer> [] adj, int v, int [] ops, Pair<Integer, Integer> [] lr){
        long sum = 0;
        for(int j : adj[v]){
            sum += dfs(adj, j, ops, lr);
        }
        if(sum < lr[v].f){
            ops[0]++;
            return lr[v].s;
        }
        return Math.min(sum, lr[v].s);
    }
