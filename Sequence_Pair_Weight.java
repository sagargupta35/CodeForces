...
public class CodeChef3 {
    ...
    static long f(List<Integer> list, int s){
        long suf = 0;
        long sum = 0;
        int n = list.size();
        for (Integer j : list) {
        ...
        for(int i = 0; i<n; i++){
            suf -= list.get(i);
            sum += (list.get(i)+1)*(((long) (n - i - 1) *s) - suf);
        }
        return sum;
    }

    static long [] idx(int [] arr, int st, long prev){
        long sum = 0;
        while (st < arr.length){
            sum += arr[st];
            if(sum >= prev) break;
        ...
        if(sum < prev) return new long[]{-1};
        return new long[]{st, sum};
    }
