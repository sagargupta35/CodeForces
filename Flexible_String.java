...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            System.out.println(solve(list, 0, 0, m, a, b));
        }
        out.close();
    }

    static long solve(List<Character> list, int st, int mask, int left, String a, String b){
        if(st == list.size() || left == 0){
            return count(a, b, mask);
        }
        int idx = list.get(st) - 'a';
        idx = 1 << idx;
        long taken = solve(list, st+1, mask^idx, left-1, a, b);
        long not = solve(list, st+1, mask, left, a, b);
        return Math.max(taken, not);
    }

    static long count(String a, String b, int mask){
        int cnt = 0;
        long sum = 0;
        for(int i = 0; i<a.length(); i++){
            int idx = a.charAt(i) - 'a';
            idx = 1 << idx;
            if(a.charAt(i) == b.charAt(i) || ((mask&idx) > 0)) cnt++;
            else{
    ...
    public void populateNext(Node root) {
       // code here
        if(root == null) return;
        if(root.left != null){
            Node right = rightMost(root.left);
            right.next = root;
            populateNext(root);
        }
        if(root.right != null){
