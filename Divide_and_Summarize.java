...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            Arrays.sort(arr);
            generate(arr, 0, arr.length-1, set);
            while (q-- > 0){
                long s = in.nextLong();
                if(set.contains(s)){
    ...
    static void generate(int [] arr, int i, int j, Set<Long> set){
        if(i > j) return;
        if(i == j){
            set.add((long) arr[i]);
            return;
        ...
        set.add(sum);
        int idx = lowerBound(arr, (arr[i]+arr[j])/2, i, j);
        if(idx != j){
            generate(arr, i, idx, set);
            generate(arr, idx+1, j, set);
        }
    }

    static int lowerBound(int [] arr, long target, int low, int high){
        int ans = low;
        while (low <= high){
            int mid = (low + high)/2;
            if(arr[mid] <= target){
