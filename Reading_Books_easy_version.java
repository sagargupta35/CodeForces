...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        PriorityQueue<Integer> bo = new PriorityQueue<>();
        for (int i = 0; i < n; i++) {
            if(a[i] == 1 && b[i] == 0) al.add(t[i]);
            else if(a[i] == 0 && b[i] == 1) bob.add(t[i]);
            else if(a[i] == 1 && b[i] == 1) bo.add(t[i]);
        }
        int s1 = k, s2 = k;
        long sum = 0;
        boolean flag = true;
        while (s1 > 0 && s2 > 0){
            if(!bo.isEmpty()){
                if(al.isEmpty() || bob.isEmpty()){
                    s1--; s2--;
                    sum += bo.poll();
                } else{
                    if(bo.peek() < al.peek() + bob.peek()){
                        sum += bo.poll();
                    } else sum += al.poll() + bob.poll();
                    s1--; s2--;
                }
            } else{
                if(al.isEmpty() || bob.isEmpty()){
