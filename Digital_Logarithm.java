...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            //merge elements of a into map1
            // remove common elements of b from a;
            //convert remaining to their scores
            //remove common elements again
            // add the sizes of remaining
            for(int j : arr) map1.merge(j, 1, Integer::sum);
            for(int j : b) map2.merge(j, 1, Integer::sum);
            for(int j : arr){
                int j1 = map1.getOrDefault(j, 0);
                int j2 = map2.getOrDefault(j, 0);
                map1.computeIfPresent(j, (k, v) -> (v - Math.min(j1, j2)));
                map2.computeIfPresent(j, (k, v) -> (v - Math.min(j1, j2)));
            }
            long sum = 0;
            for(int j : arr){
                int f = map1.getOrDefault(j, 0);
                if(j >= 10 && f > 0){
                    int s = getScore(j);
                    sum += f;
                    map1.merge(s, f, Integer::sum);
                    map1.remove(j);
                }
            }
            for(int j : b){
                int f = map2.getOrDefault(j, 0);
                if(j >= 10 && f > 0){
                    int s = getScore(j);
                    sum += f;
                    map2.merge(s, f, Integer::sum);
                    map2.remove(j);
                }
            }
            for(int j = 1; j<=9; j++){
                int v1 = map1.getOrDefault(j, 0);
                int v2 = map2.getOrDefault(j, 0);
                map1.computeIfPresent(j, (k, v) -> v - Math.min(v1, v2));
                map2.computeIfPresent(j, (k, v) -> v - Math.min(v1, v2));
            }
            for(var entry : map1.entrySet()){
                if(entry.getKey() != 1) sum += entry.getValue();
            }
