...
public class Main {
    ...
    static long dfs(int vertex, Long [] cost, long [] arr, List<Integer> [] edges){
        if(cost[vertex] != null) return cost[vertex];
        long cur = 0;
        for(int j : edges[vertex]){
            cur += dfs(j, cost, arr, edges);
        }
        return cost[vertex] = Math.min(cur, arr[vertex]);
    }
