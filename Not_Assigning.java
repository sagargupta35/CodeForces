...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            for (int i = 0; i < n - 1; i++) {
                ...
                if(flag) flag = fill(arr, u, v);
                if(flag) flag = fill(arr, v, u);
            }
            if(!flag){
            ...
            vis[0] = true;
            vis[root] = true;
            arr[root][2] = 2; arr[root][3] = 5;

            dfs(arr, false, vis, arr[root][0]);
            dfs(arr, true, vis, arr[root][1]);

            fillWeight(arr, arr[root][0], root, 2);
            fillWeight(arr, arr[root][1], root, 5);

            for(int [] edge : edges){
                int u = edge[0], v = edge[1];
                for (int i = 0; i < 2; i++) {
                    if(arr[u][i] == v){
                        System.out.print(arr[u][i+2] + " ");
                    }
                }
            }
    ...
    static void dfs(int [][] arr, boolean flag, boolean [] vis, int st){
        vis[st] = true;
        for (int i = 0; i < 2; i++) {
            if(!vis[arr[st][i]]){
                int v = arr[st][i];
                int w = flag ? 2 : 5;
                arr[st][i+2] = w;
                fillWeight(arr, v, st, w);
                dfs(arr, !flag, vis, v);
            }
        }
    }

    static boolean fill(int [][] arr, int u, int v){
        for (int i = 0; i <2; i++) {
            if(arr[u][i] == 0){
                arr[u][i] = v;
                return true;
    ...
    static void fillWeight(int [][] arr, int u, int v, int wt){
        for (int i = 0; i < 2; i++) {
            if(arr[u][i] == v){
                arr[u][i+2] = wt;
            }
