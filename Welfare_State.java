    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        for (int i = 1; i <= q; i++) {
            ...
            if(t == 1){
                ...
                list.add(new Pair<>(i, in.nextInt()));
            }
        }
        for (int i = list.size()-2; i >= 0 ; i--) {
            list.get(i).s = Math.max(list.get(i).s, list.get(i+1).s);
        }
        for (int i = 0; i < n; i++) {
            int j = binarySearch(list, f[i]);
            arr[i] = Math.max(arr[i], j);
        }
        for(int j : arr) System.out.print(j + " ");
        System.out.println();
    }

    static int binarySearch(List<Pair<Integer, Integer>> list, int target){
        int lo = 0, hi = list.size()-1, ans = -1;
        while (lo <= hi){
            int mid = lo + (hi-lo)/2;
            if(list.get(mid).f >= target){
                ans = list.get(mid).s;
                hi = mid-1;
            } else lo = mid+1;
        }
