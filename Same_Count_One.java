...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            for(int [] cur : arr){
                for(int j : cur) if(j == 1) total++;
            }
            if(total % n != 0) System.out.println(-1);
            else{
                    ...
                    while (!max.isEmpty()){
                        ...
                        if(up[i] == 1){
                            while (!min.isEmpty()){
                                ...
                                if(dow[i] == 0){
                                    // performing a swap
                                    System.out.println((j+1) + " " + (j2+1) + " " + (i+1));
                                    ma[j]--;
                                    mi[j2]++;
                                    if(mi[j2] < each) q2.add(j2);
                                    break;
                                } else{
                                    q2.add(j2);
                                }
                            }
                            if(ma[j] > each) q1.add(j);
                        } else q1.add(j);
                    }
                    while (!q1.isEmpty()) max.add(q1.poll());
                    while (!q2.isEmpty()) min.add(q2.poll());
                }
            }
        }
        out.close();
