...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            long [] arr = al(n);
            Arrays.sort(arr);
            if(isInfinity(arr, days, c)) System.out.println("Infinity");
            else if((arr[n-1]*days) < c) System.out.println("Impossible");
            else{
                int max = 0;
                int lo = 0, hi = days;
                while (lo <= hi){
                    int mid = lo + (hi-lo)/2;
                    if(canCool(arr, mid, days, c)){
    ...
    static boolean isInfinity(long [] arr, int days, long c){
        int n = arr.length;
        int left = Math.min(days, n);
        long sum = 0;
        for(int i = 1; i<=left; i++) sum += arr[n-i];
        return sum >= c;
    }

    static boolean canCool(long [] arr, int mid, int days, long c){
        int n = arr.length;
        int cool = mid+1;
        int more = Math.min(n, days%cool);
        int left = Math.min(cool, n);
        int times = (days/cool);
        long sum = 0;
