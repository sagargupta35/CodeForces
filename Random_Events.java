...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        int t = in.nextInt();
        while (t-- > 0){
            double prob = 1.0000000;
            int n = in.nextInt();
            int m = in.nextInt();
            int [] arr = ai(n);
            int st = getLastInt(arr);
            if(st == 0){
            ...
            System.out.printf("%.6f%n", (1-prob));
        }
        out.close();
    }

    static int getLastInt(int [] arr){
        int st = arr.length;
        while (st > 0 && arr[st-1] == st) st--;
        return st;
    }

    static boolean isPossible(int max, int [] bloomDay, int m, int k){
        int sum = 0, cur = 0;
        for (int j : bloomDay) {
            if (j <= max) {
                cur++;
    ...
    static long getNc3(long n){
        n = (n * (n-1) * (n-2));
        return n/6;
    }

    static boolean areEqual(long [] arr){
        for(long j : arr){
            if(j != 0) return false;
        }
        return true;
    ...
    static int getNextIdx(int st, int end, String s, char c){
        while (end >= st){
            if(s.charAt(end) == c){
                break;
            }
