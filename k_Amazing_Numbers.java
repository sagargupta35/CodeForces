...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            Arrays.fill(prevs, -1);
            for(int i = 0; i<n; i++){
                mp[arr[i]] = Math.max(mp[arr[i]], i - prevs[arr[i]] - 1);
                prevs[arr[i]] = i;
            }
            for(int i = 1; i<=n; i++){
                mp[i] = Math.max(mp[i], n - prevs[i] - 1);
            }
    ...
    static long [] getArr(long a, long b, long c, long d){
        for(long y = b+1; y<=d; y++){
            long x = lcm(a*b, y)/y;
            x = (c/x)*x;
            if(a < x && x <= c) return new long [] {x, y};
        }
        return new long [] {-1, -1};
    }
