...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        int k = in.nextInt();
        int [][] grid = new int [n][];
        for(int i = 0; i<n; i++) grid[i] = ai(m);
        int [] vis = new int [m];
        for(int i = 0; i<m; i++) isDown(grid, i, vis);
        while (k-- > 0){
            int col = in.nextInt()-1;
            int j = drop(grid, vis, 0, col);
    ...
    static int drop(int [][] grid, int [] vis, int i, int j){
        int n = grid.length;
        if(i == n) return j;
        if(vis[j] == n) return j;
        if(grid[i][j] == 2) return drop(grid, vis, i+1, j);
        vis[j]++;
        if(grid[i][j] == 1){
            grid[i][j] = 2;
            return drop(grid, vis, i, j+1);
        }
        grid[i][j] = 2;
        return drop(grid, vis, i, j-1);
    }

    static void isDown(int [][] arr, int col, int [] vis){
        for (int[] ints : arr) {
            if(ints[col] == 2) vis[col]++;
        }
    }
