...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0) {
            ...
            for(int i = 0; i<10; i++) map.put(i, new TreeSet<>());
    ...
    static void f(StringBuilder sbr, String s, int i, int k, Map<Integer, TreeSet<Integer>> map, int next){
        if(k == s.length()-i) return;
        if(k == 0){
            if(i >= s.length()) return;
            sbr.append(s.substring(i));
            return;
        }
        while (next < 10){
            Integer ceil = map.get(next).ceiling(i);
            if(ceil != null && ceil <= i+k){
                sbr.append(next);
                f(sbr, s, ceil+1, k - (ceil-i), map, 0);
                break;
            }
            next++;
        }
