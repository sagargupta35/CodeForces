    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0) {
            ...
            for(int i = 0; i<n; i++){
                int st = 0;
                for(int j = 0; j<m; j++){
                    if(arr[seat[i][j]] != arr[seat[i][st]]){
                        reverse(seat[i], st, j-1);;
                        st = j;
                    }
                }
                reverse(seat[i], st, m-1);
            }
//            System.out.println(Arrays.deepToString(seat));
            int sum = 0;
            for(int i = 0; i<n; i++){
                int j = sort(seat[i], 0, m-1);
//                System.out.println(j);
    ...
    static int merge(int [] arr, int st, int mid, int end){
        int [] l = new int [mid - st + 1];
        int [] r = new int [end - mid];
        if (mid + 1 - st >= 0) System.arraycopy(arr, st, l, st - st, mid + 1 - st);
