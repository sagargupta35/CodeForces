...
public class CodeChef3 {
        ...
        while (t-- > 0){
            ...
            for(int i = 1; i<=Math.min(n, 100); i++){
                ...
                for(int j = 0; j<i; j++){
                    ...
                    if(arr[num] == 0) distinct++;
                    max = Math.max(max, ++arr[num]);
                }
                if(max <= distinct) sum++;
                for(int j = i; j<n; j++){
                    int n1 = s.charAt(j-i)-'0';
                    if(--arr[n1] == 0) distinct--;
                    int num = s.charAt(j)-'0';
                    if(arr[num] == 0) distinct++;
                    arr[num]++;
                    max = aMax(arr);
    ...
    public static double findSmallestMaxDist(int[] stations, int k) {
        ...
        int [] arr = new int [n-1];
        Comparator<Pair<Double, Integer>> c = (p1, p2) -> {
            if(p1.f > p2.f) return -1;
            return 1;
        };
        PriorityQueue<Pair<Double, Integer>> pq = new PriorityQueue<>(c);
        double max = 0;
        for(int i = 0; i<n-1; i++){
        ...
        for(int i = 0; i<k; i++){
            var p = pq.poll();
            arr[p.s]++;
            double dist = stations[p.s+1] - stations[p.s];
            dist /= arr[p.s];
            p.f = dist;
            pq.add(p);
        }
