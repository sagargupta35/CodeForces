...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            while (min <= max){
                ...
                long cur = check(pre, mid, x, a, y, b);
                if(cur >= k){
                    res = mid;
                    max = mid-1;
                } else min = mid+1;
    ...
    static long check(long [] pre, int tickets, int x, int a, int y, int b){
        int n = pre.length;
        long lcm = lcm(a, b);
        int both = (int)  (tickets/lcm);
        int max = (x > y) ? tickets/a - both : tickets/b - both;
        int min = (x > y) ? tickets/b - both : tickets/a - both;
        long sum = 0;
        int cur = n-1;
        if(both > 0){
            long temp = pre[cur];
            if(cur - both >= 0) temp -= pre[cur-both];
            temp /= 100;
            temp *= (x+y);
            sum += temp;
        }
