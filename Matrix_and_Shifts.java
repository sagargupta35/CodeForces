...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0) {
            ...
            for (int i = 0; i < n; i++) {
                for (int j = 0; j < n; j++) {
                    row[i] += mat[i].charAt(j)-'0';
                }
            }
            for (int i = 0; i < n; i++) {
                ans = Math.min(ans, getOps(mat, 0, i, row));
            }
    ...
    static int getOps(String [] mat, int i, int j, int [] row){
        int n = mat.length;
        int ans = 0;
        while(n-- > 0){
            if(mat[i].charAt(j) == '1'){
    ...
    public static int maxMeetings(int n, int start[], int end[]) {
        ...
        Arrays.sort(pairs, Comparator.comparingInt(p -> p.f));
        int cnt = 1, max = pairs[0].s;
        for(int i = 1; i<n; i++){
            if(pairs[i].f > max){
                cnt++;
                max = pairs[i].s;
            } else max = Math.min(max, pairs[i].s);
        }
        return cnt;
    }
