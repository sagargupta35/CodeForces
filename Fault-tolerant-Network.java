...
public class Main {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0) {
            ...
            long [] bn = new long [2];
            for(int i = 0; i<2; i++){
                an[i] = getNearest(brr, a[i]);
                bn[i] = getNearest(arr, b[i]);
            }
            System.out.println(Math.min(getMin(a, b, an, bn, new boolean [2] , new int [2]), getMin(b, a, bn, an, new boolean [2] , new int [2])));
        }
        out.close();
    }

    static long getMin(long [] a, long [] b, long [] an, long [] bn, boolean [] af, int [] bf){
        ...
        for(int i = 0; i<2; i++){
            if(!af[i]){
                ...
                for(int j = 0; j<2; j++){
                    bf[j]++;
                    sum = Math.min(sum, Math.abs(a[i]-b[j])+getMin(a, b, an, bn, af, bf));
                    bf[j]--;
                }
                sum = Math.min(sum, an[i] + getMin(a, b, an, bn, af, bf));
                af[i] = false;
            }
        }
        if(af[0] && af[1]){
    ...
    static long getNearest(long [] arr,long target){
