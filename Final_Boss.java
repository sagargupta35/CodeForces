...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            long lo = 1, hi = 100000000000000000L;
            while (lo <= hi){
                long mid = lo + (hi - lo)/2;
                if(isPossible(arr, c, mid, h)) {
    ...
    static boolean isPossible(int [] arr, int [] c, long turns, int h){
        long sum = 0;
        int n = arr.length;
        for(int i = 0; i<n && sum < h; i++){
            if((c[i] - 1 + turns) / c[i] >= h){
                return true;
            }
            sum += (long) ((c[i] - 1 + turns) / c[i]) * arr[i];
        }
        return sum >= h;
    }
