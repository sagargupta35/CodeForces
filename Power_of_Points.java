...
public class Main {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0) {
            ...
            for(int i = 0; i<n; i++) map.merge(arr[i], 1, Integer::sum);
            int [] sorted = new int [map.size()];
            int st = 0;
            for(int k : map.keySet()) sorted[st++] = k;
            int n2 = sorted.length;
            long [] left  =new long [n2];
            long [] right = new long [n2];
            long f = map.get(sorted[0]);
            for(int i = 1; i<n2; i++){
                left[i] = left[i-1] + (sorted[i] - sorted[i-1]) * f;
                f += map.get(sorted[i]);
            }
            f = map.get(sorted[n2-1]);
            for(int i = n2-2; i>=0; i--){
                right[i] = right[i+1] + (sorted[i+1] - sorted[i]) * f;
                f += map.get(sorted[i]);
            }
            for(int i = 0; i<n; i++){
                int idx = binarySearch2(sorted, arr[i]);
                System.out.print((right[idx] + left[idx] + n) + " ");
            }
            System.out.println();
        }
        out.close();
